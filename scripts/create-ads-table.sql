...
-- Policy for authenticated users to read ads
CREATE POLICY "Allow authenticated users to read ads" ON public.ads
FOR SELECT USING (auth.role() = 'authenticated');

-- Policy for authenticated users to insert ads
CREATE POLICY "Allow authenticated users to insert ads" ON public.ads
FOR INSERT WITH CHECK (auth.role() = 'authenticated');

-- Policy for authenticated users to update ads
CREATE POLICY "Allow authenticated users to update ads" ON public.ads
FOR UPDATE USING (auth.role() = 'authenticated');

-- Policy for authenticated users to delete ads
