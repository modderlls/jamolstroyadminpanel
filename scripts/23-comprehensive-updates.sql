-- Comprehensive updates for JamolStroy platform
-- This script includes all the requested features and fixes

-- Add oauth_clients table for login validation
...
INSERT INTO oauth_clients (client_id, client_secret, name, is_active) VALUES
('jamolstroy_web_app', 'js_secret_2024', 'JamolStroy Web Application', true)
ON CONFLICT (client_id) DO NOTHING;

-- Add delivery limit fields to products table
ALTER TABLE products ADD COLUMN IF NOT EXISTS delivery_limit_quantity numeric DEFAULT 0;
ALTER TABLE products ADD COLUMN IF NOT EXISTS delivery_price_over_limit numeric DEFAULT 0;

-- Update products with delivery limits
UPDATE products SET 
    delivery_limit_quantity = 10,
    delivery_price_over_limit = delivery_price * 1.5
WHERE has_delivery = true AND delivery_limit_quantity = 0;
...
BEGIN
    RETURN QUERY
    SELECT sq.query, sq.search_count
...
-- Function to calculate delivery fee with free delivery threshold
...
-- Grant permissions
GRANT SELECT ON oauth_clients TO anon, authenticated, service_role;
...
-- RLS policies
CREATE POLICY "OAuth clients are viewable by everyone" ON oauth_clients FOR SELECT USING (true);
